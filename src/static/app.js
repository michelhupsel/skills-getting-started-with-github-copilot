      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = participants.length
          ? `<ul class="participants-list">${participants
              .map(
                (p) =>
                  `<li class="participant-item">${p}<button class="remove-participant" data-activity="${encodeURIComponent(
                    name
                  )}" data-email="${encodeURIComponent(p)}" title="Remove ${p}">&times;</button></li>`
              )
              .join("")}</ul>`
        ...
        // Add delete listeners for participants
